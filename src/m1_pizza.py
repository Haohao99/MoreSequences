...
def run_test_generate_points_on_circle():
    ...
    print('Testing the generate_points_on_circle    function:')
    print('--------------------------------------------------')
    expected = [rg.Point(180.0, 100.0),
                rg.Point(165.0, 126.0),
                rg.Point(135.0, 126.0),
                rg.Point(120.0, 100.0),
                rg.Point(135.0, 74.0),
                rg.Point(165.0, 74.0)]
...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    # Your professor may do this exercise with you as "live coding".
    # ------------------------------------------------------------------
    sequences = generate_points_on_circle(circle,number_of_points)
...
def run_test_pizza():
    ...
    # ------------------------------------------------------------------
    title = 'PIZZA test 4 : a large number of thin black lines one yellow circle'
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    # ------------------------------------------------------------------
    sequences = generate_points_on_circle(circle,number_of_slices)
    circle.attach_to(window)

    for k in range(len(sequences)):
        line = rg.Line(circle.center,sequences[k])
...
def run_test_polygon():
    ...
    # ------------------------------------------------------------------
    title = 'POLYGON test 3:  10 with black lines.'
    window = rg.RoseWindow(550,400,title)
...
def run_test_fancy_polygon():
    ...
    # ------------------------------------------------------------------
    title = 'FANCY POLYGON test 4:  20 red lines on green circle, hops = 5.'
    window = rg.RoseWindow(500,350,title)
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color, thickness):
    ...
    for k in range(len(sequence)):
        m = k + hops_to_next_point
        if m >= number_of_lines:
            m = m - number_of_lines
        line = rg.Line(sequence[k],sequence[m])
        line.color = color
        line.thickness = thickness
        line.attach_to(window)
        window.render()
